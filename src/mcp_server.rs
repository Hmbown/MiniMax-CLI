//! MCP server implementation for exposing MiniMax tools over stdio.
    ...
    fn list_resources_response(&self) -> Value {
            ...
            && let Ok(sessions) = manager.list_sessions()
        {
            for session in sessions {
                resources.push(json!({
                    "uri": format!("minimax://session/{}", session.id),
                    "name": session.title,
...
fn default_config_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".minimax").join("mcp_server.toml"))
}
